...
import { WebScrapingRequest, WebScrapingResponse } from '@/types';
...
export const WebScraper: React.FC<WebScraperProps> = ({ onScrape }) => {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const response: WebScrapingResponse = await apiService.scrapeWebsite(request);
      setResult(response);
      ...
      <div className="space-y-6">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-green-800 space-y-1">
            <li>• 支持大部分网站的内容抓取</li>
            <li>• 智能摘要模式可以提取关键信息</li>
            <li>• 可以通过自定义提示指定特定的抓取要求</li>
            <li>• 抓取的内容会自动进行清理和格式化</li>
          </ul>
        </div>
      </div>
    </Card>
